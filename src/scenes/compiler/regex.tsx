...
export class RegexNode extends Circle {
  ...
  constructor(props: RegexNodeProps) {
    ...
    if (!props.size) props.size = REGEX_NODE_SIZE;
    ...
    this.add(
      <Txt ref={this.text} fill={theme.primary} fontSize={48}>
        {note}
      </Txt>
    );
  }
...
export abstract class Regex extends Rect {
  ...
  protected abstract get subs(): Regex[];
  protected abstract get lines(): Line[];
...
export class AcceptRegex extends Regex {
  ...
  // private readonly afterLines = makeRefs<>();
  ...
  constructor(props?: ConcatRegexProps) {
    ...
    this.add([
      Regex.From({
        ...this.lhs(),
        ref: this.lhsRegex,
        marginTop: REGEX_NODE_SIZE + REGEX_GAP,
        theme: this.theme,
      }),
